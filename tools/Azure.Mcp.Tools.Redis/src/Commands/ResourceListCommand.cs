...
public sealed class ResourceListCommand(ILogger<ResourceListCommand> logger) : SubscriptionCommand<ResourceListOptions>()
{
    ...
    public override async Task<CommandResponse> ExecuteAsync(CommandContext context, ParseResult parseResult)
    {
        ...
        try
        {
            ...
            var caches = await redisService.ListResourcesAsync(
                options.Subscription!,
                options.Tenant,
                options.AuthMethod,
                options.RetryPolicy);

            context.Response.Results = ResponseResult.Create(new(caches ?? []), RedisJsonContext.Default.ResourceListCommandResult);
        }
        catch (Exception ex)
        {
